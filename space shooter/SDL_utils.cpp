...
/**
* Nạp một ảnh vào một texture trên thiết bị hiện thị (rendering device)
...
SDL_Texture* loadTexture(const std::string &file, SDL_Renderer *ren)
{
	...
	if (loadedImage != nullptr){
		...
		logSDLError(std::cout, "IMGLoad");
	}
	return texture;
}

/**
* Vẽ một SDL_Texture từ font có trước,
* @param font: Phông chữ có trước
* @param text: String mà ta cần vẽ
* @param renderer: renderer để nạp texture lên
* @param textColor: Màu của chữ cần vẽ
*/
SDL_Texture* loadText(TTF_Font* font, std::string text, SDL_Color textColor, SDL_Renderer* renderer)
{
    ...
    SDL_Surface* loadedText = TTF_RenderText_Blended(font, text.c_str(), textColor);
    if (loadedText != nullptr)
    {
        textureText = SDL_CreateTextureFromSurface(renderer, loadedText);
        SDL_FreeSurface(loadedText);
        if (textureText == nullptr)
            logSDLError(std::cout, "CreateTextureFromSurface", true);
    }
    else logSDLError(std::cout, "Loading text", true);
    return textureText;
}
...
* từ hình chữ nhật là 1 phần của texture nguồn
* @param tex: texture nguồn chúng ta muốn vẽ ra
* @param ren: thiết bị renderer chúng ta muốn vẽ vào
* @param source: hình chữ nhật là 1 phần của texture
* @param x: hoành độ
* @param y: tung độ
* @param w: chiều rộng (mới)
* @param h: độ cao (mới)
